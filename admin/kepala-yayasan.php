			...
			<div class="box-body">
				...
				<form action="" method="POST" enctype="multipart/form-data">
					...
					<div class="form-group">
						<label>Nama</label>
						<input type="text" name="nama" class="input-control" placeholder="Nama Kepala Yayasan" value="<?= $d->nama_kepsek ?>" required>
					</div>

					<div class="form-group">
						<label>Sambutan</label>
						<textarea name="sambutan" class="input-control" placeholder="Sambutan Kepala Yayasan" id="keterangan"><?= $d->sambutan_kepsek ?></textarea>
					</div>
				...
				if (isset($_POST['submit'])) {
					...
					if ($_FILES['foto_baru']['name'] != '') {
						...
						if (!in_array($formatfile, $allowedtype)) {
							...
							echo '<div class="alert alert-error">Format file foto kepala yayasan tidak diizinkan.</div>';

							return false;
						} elseif ($filesize > 1000000) {

							echo '<div class="alert alert-error">Ukuran file foto kepala yayasan tidak boleh lebih dari 1 MB.</div>';
					...
					if ($update) {
						echo "<script>window.location='kepala-yayasan.php?success=Edit Data Berhasil'</script>";
					} else {
